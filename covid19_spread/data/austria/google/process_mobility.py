...
def get_county_mobility_google(fin=None):
    ...
    df_Gmobility = df_Gmobility_global.query("country_region_code == 'AT'")
    return df_Gmobility
...
df = df.rename(columns={"Politischer Bezirk": "region"}).drop(columns=["Bundesland"])
df = df[["region", "type"] + list(df.columns[1:-1])]

df.round(4).to_csv("mobility_features_google.csv", index=False)
print(df)
