...
# select regin or province
link = "https://raw.githubusercontent.com/pcm-dpc/COVID-19/master/dati-regioni/dpc-covid19-ita-regioni.csv"
...
_nodes = data.denominazione_regione.unique()
nodes = list(_nodes)
rows = []
...
for _date in _dates:
    ...
    assert (nodes == row["denominazione_regione"].values).all()

    row_tmp += list(row["totale_positivi"].values)
    rows.append(row_tmp)
        ...
        dataset.append(
            {
                "date": date,
                "days_since_outbreak": (
                    date_to_timestamp(date) - date_to_timestamp(first_outbreak_day)
                )
                / (60 * 60 * 24),
                "node_idx": node_idx,
