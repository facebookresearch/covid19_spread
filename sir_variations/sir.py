...
def main(args):
    ...
    parser.add_argument(
        "-doubling-times", type=float, nargs="+", help="Addl d-times to simulate"
    )
    ...
    parser.add_argument(
        "-bc-window", type=int, default=5, help="beta constant averaging window"
    )
    parser.add_argument(
        "-fit-window", type=int, default=10, help="fit using last .. days"
    )
    parser.add_argument(
        "-initial-state", type=int, default=0, help="days of rewinding for retrofit"
    )
    parser.add_argument(
        "-coeff",
        type=float,
        default=1.0,
        help="between 0 & 1, fraction of reported cases",
    )
