...
analyze-nj: fdata = data/new-jersey/timeseries.h5
analyze-nj:
	$(call _analyze_sweep)
...
data-nj: data/new-jersey/nj-official-$(shell date "+%m%d" -d $(DATE)).csv
	cd data/new-jersey && make data-$(DATE).csv && make timeseries.h5
...
grid-nys: doubling-times = 10 11 12 13
grid-nys:
...
analyze-nyc: fdata = data/nystate/timeseries-nyc.h5
analyze-nyc:
	$(call _analyze_sweep)


analyze-nys: sweepdir = $(shell tail -$(LAST) runs/nys/$(DATE).log | head -n1)
analyze-nys: fdata = data/nystate/timeseries-nys.h5
analyze-nys:
	$(call _analyze_sweep)
...
data-ny:
	cd data/nystate && make data-nj.csv && make timeseries-nyc.h5 timeseries-nys.h5
