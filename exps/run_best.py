...
def launch_best(sweep_dir):
    ...
    for d in os.listdir(sweep_dir):
        if re.match('\d+_\d+', d):
            ...
            log = check_output(f'cat {os.path.join(sweep_dir,d)}/{d}*.out | grep json_stats | grep -o "\\{{.*\\}}"', shell=True)
    ...
    launched = launch_job(os.path.dirname(df.sort_values(by=mae_col).iloc[0].pth), f'best_{mae_col}', set())


    # Launch job by best KS
    launched = launch_job(os.path.dirname(df.sort_values(by='ks').iloc[0].pth), 'best_ks', launched)

    # Launch job by best pval
    launched = launch_job(os.path.dirname(df.sort_values(by='pval').iloc[-1].pth), 'best_pval', launched)

    # Launch job by best LL
    launched = launch_job(os.path.dirname(df.sort_values(by='best_ll').iloc[-1].pth), 'best_ll', launched)
