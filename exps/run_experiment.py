...
def run_experiment(
    ...
    if crossval:
        job_dset = os.path.join(
            job_dir, "../", os.path.basename(pdict["dset"]) + f".minus_{days}_days"
        )
    else:
    ...
    train.main(
        [
            ...
            for x in [
                ...
                pdict.get("lr"),
                "-epochs",
                pdict.get("epochs", 100),
                "-max-events",
                pdict.get("max-events", 500000),
                "-checkpoint",
                job_checkpoint,
                "-timescale",
                pdict.get("timescale", 1),
                "-scale",
                pdict.get("scale", 1),
                "-sparse",
                "-optim",
                pdict.get("optim", "adam",),
                "-weight-decay",
                pdict.get("weight-decay", 0),
                "-const-beta",
                pdict.get("const-beta", -1),
                "-lr-scheduler",
                pdict.get("lr-scheduler", "constant"),
                "-no-sparse-grads",
            ]
    ...
    rmse(days, job_checkpoint, prefix=prefix, max_events=max_sim_events)
