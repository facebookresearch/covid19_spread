...
def launch(grid, exp_name, crossval, dicts, now, opt):
    ...
    for i, dset in enumerate(grid["dset"]):
        if opt.backfill is not None:
            backfill = pandas.Timestamp(opt.backfill)
            with tempfile.NamedTemporaryFile() as tfile, h5py.File(dset, "r") as hf:
                basedate = pandas.Timestamp(hf.attrs["basedate"])
                assert basedate > backfill, "Can't backfill a date in the future!"
                drop_k_days(dset, tfile.name, (basedate - backfill).days)
                shutil.copy(tfile.name, os.path.join(base_dir, os.path.basename(dset)))
        else:
            shutil.copy(dset, os.path.join(base_dir, os.path.basename(dset)))

    if crossval:
        for i, dset in enumerate(grid["dset"]):
            copied = os.path.join(base_dir, os.path.basename(dset))
            outfile = os.path.join(
                base_dir, os.path.basename(dset) + f".minus_{opt.days}_days"
            )
            drop_k_days(copied, outfile, opt.days)
    ...
    with snapshot.SnapshotManager(
        snapshot_dir=base_dir + "/snapshot", with_submodules=True
    ):
        jobs = executor.map_array(
            partial(run_experiment, grid, opt.days, crossval, folder), dicts
        )
    print(folder[:-3])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-backfill", help="Date to backfill")
    parser.add_argument("-max-sim-events", type=int, default=2000000)
    ...
    if opt.deaths:
        ...
        grid["dset"] = [
            os.path.realpath(
                os.path.join(
                    ...
                    f"../data/usa/timeseries_smooth_1_days_mode_deaths.h5",
                )
            )
        ]
    else:
        grid["dset"] = [
            os.path.realpath(
                os.path.join(
                    os.path.dirname(__file__),
                    f"../data/usa/timeseries_smooth_1_days_mode_adjacent_states.h5",
                )
            ),
            os.path.realpath(
                os.path.join(
                    os.path.dirname(__file__),
                    f"../data/usa/timeseries_smooth_2_days_mode_adjacent_states.h5",
                )
            ),
            os.path.realpath(
                os.path.join(
                    os.path.dirname(__file__),
                    f"../data/usa/timeseries_smooth_3_days_mode_adjacent_states.h5",
                )
            ),
        ]
        opt.max_sim_events = 3000000 if opt.max_sim_events < 0 else opt.max_sim_events
