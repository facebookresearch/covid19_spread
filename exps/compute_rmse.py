...
def rmse(days, pth, with_ks=True, trials=10, prefix="", max_events=-1):
    ...
    mapper = {
        d: str((basedate + pandas.Timedelta(d, "D")).date())
        for d in sim.columns
    ...
    if with_ks:
        ...
        ks_result.to_csv(
            os.path.join(os.path.dirname(pth), prefix + "kstest.csv"), index=False
        )
...
if __name__ == "__main__":
    ...
    output = subprocess.check_output(
        "sinfo -lR | grep drng | awk '/Xid/ {print $5}'", shell=True
    )
